...
prior_three_year_experience_yn = false
...
신인활동지원비를 `EXCLUDED`로 넣는 시드는 경력조회·등록일·지원근거 `evidence_ref`를 반드시 가진다.
...
- 84개월 범위, 각 종류별 동일 월금액
- 61~84차월은 유지관리 24,000 + 장기유지 12,000 = 월 36,000원이 동시 지급된다
- **두 수수료의 월 상한을 각각 따로 비교한다.** 합산액 36,000원(월 1.2%)을 하나의 상한과 비교하지 않는다
- 장기유지 61차월 시작은 REG-05의 “계약이 5년 이상 유지” 문언을 60개월 경과 후로 옮긴 FGC 정책이다. 49차월은 5년차 시작일일 뿐 5년 유지 완료시점이 아니므로 허용 시작회차로 사용하지 않는다
- 최초 지급회차와 월중 해지·실효·부활 기준은 최종 FAQ 전문 확보 전 `PROJECT_ASSUMPTION`으로 버전 관리한다
- 세 한도를 서로 차감하지 않는다
...
2차 시드에는 실제 공개 환수표를 그대로 쓰지 않고 단순화한 가상 그룹을 사용한다.

| 그룹 | 1~3회 | 4~6회 | 7~12회 | 13~18회 | 19회 이후 |
|---|---:|---:|---:|---:|---:|
| A | 100% | 90% | 60% | 30% | 0% |
| B | 100% | 80% | 50% | 20% | 0% |
| C | 100% | 70% | 40% | 10% | 0% |

실제 보험회사별 환수율을 반영할 때는 새 `policy_version`을 만들고 상품·항목·계약일·경과회차 조건을 모두 저장한다.
...
| REG-21 | 등록일·직전 3년 경력조회·지원기간·증빙 | 적격 제외와 경력 정착지원을 분리 |
...
본 문서는 `v1.0 규제·스키마 정합성 보정본`이며 팀에 배포한다.
