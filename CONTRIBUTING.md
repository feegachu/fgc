...
```text
[Feat] FUN-053 예외 조회 및 처리 워크플로 구현
```
...
```text
[Bug] 사이드바 하위 메뉴 초기화 문제 수정
```

#### 문서 수정

```text
[Docs] 인터페이스 정의서 API 검색 조건 및 화면정의서 수정
```

#### 리팩터링

```text
[Refactor] 계약 조회 서비스 검색 조건 처리 구조 개선
```
...
| 유형 | 설명 |
|---|---|
| `[Feat]` | 새로운 기능 또는 API 구현 |
| `[Fix]` | 기존 기능 오류 수정 |
| `[Bug]` | 버그 수정 |
| `[Docs]` | 문서 추가 및 수정 |
| `[Refactor]` | 기능 변경 없는 코드 구조 개선 |
| `[Test]` | 테스트 코드 추가 및 수정 |
| `[Style]` | 포맷, 명칭, UI 스타일 수정 |
| `[Chore]` | 설정, 빌드, 의존성 등 기타 작업 |

> 가능하면 관련 기능번호(`FUN-033`), 화면번호(`CONT-W01`), 인터페이스번호(`IF-API-24`)를 제목에 함께 작성합니다.
