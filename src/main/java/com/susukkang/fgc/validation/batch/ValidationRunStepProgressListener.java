...
public class ValidationRunStepProgressListener implements StepExecutionListener {
    ...
    public ValidationRunStepProgressListener(int stepNo, boolean initialStep,
                                              ValidationRunBatchProgressService progressService,
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        if (stepExecution.getStatus() == BatchStatus.COMPLETED) {
            if (initialStep) {
                ...
                progressService.startRunning(validationRunId);
                auditService.recordStarted(validationRunId, parameters);
            } else {
                // 2~8 성공 = RUNNING 상태를 유지한 채 current_step만 이 Step의 번호로 전진
                progressService.advanceStep(validationRunId, stepNo);
            }
        } else {
            // 실패 사유는 이 Step에서 던진 예외들의 메시지를 모아 남김
            String failureMessage = summarizeFailure(stepExecution);
            progressService.markFailed(validationRunId, stepNo, failureMessage);
            auditService.recordFailed(validationRunId, parameters, failureMessage);
        }
        return stepExecution.getExitStatus();
    }
