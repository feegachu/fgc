...
public interface ValidationRunBatchAuditService {
    ...
    /**
     * FAILED였던 실행을 RUNNING으로 되돌려 재시도할 때 남긴다(#77 — 재실행 시 triggeredBy·
     * requestId·실패·재시작 이력이 감사 로그에 남아야 한다는 요구사항). 이 전이는
     * lifecycleService.start()를 타지 않고 범용 ValidationRunTransitionService를 쓰기
     * 때문에(CreateDailyRunTasklet 참고), 그 호출자가 성공한 뒤 이 메서드를 직접 불러야 한다.
     */
